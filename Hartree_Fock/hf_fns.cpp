...
void double_integral::apply_symmetries(longVector& doubles, uint64_t i, uint64_t j, uint64_t k, uint64_t l, long double val) {
    ...
    if (maxIdx >= doubles.size()) {
        doubles.conservativeResize(maxIdx + 1);
    }
    
    // Assign all permutations
    for (const auto& p : perms) {
        doubles[compoundIndex(p[0], p[1], p[2], p[3])] = val;
    }
}
